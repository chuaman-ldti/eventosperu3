...
try {
    if ($method === 'GET') {
        if (isset($_GET['id'])) {
            ...
            if (!$res) resp_err("eventos no encontrado", 404);
            resp_ok($res);
        } else {
            $stmt = $conn->query("SELECT * FROM eventos ORDER BY id DESC");
    ...
    if ($method === 'POST') {
        ...
        $sql = "INSERT INTO eventos ($cols) VALUES ($place)";
        ...
        if ($stmt->execute()) {
            ...
            resp_err("Error al crear eventos", 500);
        }
    }
    ...
    if ($method === 'PUT') {
        ...
        if (!$existing) resp_err("eventos no encontrado", 404);
        $updates = [];
        foreach ($data as $k => $v) {
            $updates[] = "$k = :$k";
        }
        if (empty($updates)) resp_err("No hay campos para actualizar", 400);
        $sql = "UPDATE eventos SET " . implode(", ", $updates) . " WHERE id = :id";
        $stmt = $conn->prepare($sql);
        foreach ($data as $k => $v) {
            $stmt->bindValue(':' . $k, $v);
        }
        ...
        if ($stmt->execute()) {
            ...
            resp_err("Error al actualizar eventos", 500);
        }
    }
    ...
    if ($method === 'DELETE') {
        ...
        if ($stmt->execute()) {
            ...
            resp_err("Error al eliminar eventos", 500);
        }
    }
