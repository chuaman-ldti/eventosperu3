...
try {
    if ($method === 'GET') {
        if (isset($_GET['id'])) {
            ...
            if (!$res) resp_err("resenas no encontrado", 404);
            resp_ok($res);
        } else {
            $stmt = $conn->query("SELECT * FROM resenas ORDER BY id DESC");
    ...
    if ($method === 'POST') {
        ...
        $sql = "INSERT INTO resenas ($cols) VALUES ($place)";
        ...
        if ($stmt->execute()) {
            ...
            resp_err("Error al crear resenas", 500);
        }
    }
    ...
    if ($method === 'PUT') {
        ...
        if (!$existing) resp_err("resenas no encontrado", 404);
        $updates = [];
        foreach ($data as $k => $v) {
            $updates[] = "$k = :$k";
        }
        if (empty($updates)) resp_err("No hay campos para actualizar", 400);
        $sql = "UPDATE resenas SET " . implode(", ", $updates) . " WHERE id = :id";
        $stmt = $conn->prepare($sql);
        foreach ($data as $k => $v) {
            $stmt->bindValue(':' . $k, $v);
        }
        ...
        if ($stmt->execute()) {
            ...
            resp_err("Error al actualizar resenas", 500);
        }
    }
    ...
    if ($method === 'DELETE') {
        if (!isset($_GET['id'])) resp_err("Se requiere id", 400);
        $id = intval($_GET['id']);
        $stmt = $conn->prepare("DELETE FROM resenas WHERE id = :id");
        $stmt->bindParam(':id', $id, PDO::PARAM_INT);
        if ($stmt->execute()) {
            resp_ok(["success" => true, "id" => $id]);
        } else {
            resp_err("Error al eliminar resenas", 500);
        }
    }
